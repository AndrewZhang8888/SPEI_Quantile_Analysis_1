...
a <- select(a, ï..AvgTEMP_C, MaxTEMP_C, MinTEMP_C, Month, Day, Year)
a1 <- select(a, ï..AvgTEMP_C, Year)%>%
  group_by(Year) %>%
  summarize(average = mean(ï..AvgTEMP_C))
ab1 <- na.omit(a1)
...
ab3 <- na.omit(a3)
b1 <- pnorm(ab1$average, mean = mean(a$ï..AvgTEMP_C, na.rm = TRUE), sd = sd(a$ï..AvgTEMP_C, na.rm = TRUE))
